...
def test_total_records(data):
    ...
    print(f"✓ Total Silappadhikaram records: {len(data)}")
...
def quality_report(data):
    ...
    for item in data:
        ...
        if not item["kaandam"].strip():
            missing_kaandam += 1

        if not item["kaathai"].strip():
            missing_kaathai += 1

        if item["poemNumber"] is None:
            missing_numbers += 1
    ...
    print(f"Missing Kaandam         : {missing_kaandam}")
...
if __name__ == "__main__":
    ...
    print("\n===== TamilVerse Silappadhikaram Test =====\n")
    ...
    print("\n🎉 ALL SILAPPADHIKARAM TESTS PASSED")